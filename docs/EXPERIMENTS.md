...
- [EXP-010 robustness evidence, 30 August](evidence/EXP_010_ROBUSTNESS.md): query/corpus tokenizer asymmetry and a surface-brittle override trigger closed two of five gate failures; typo correction (+0.005) and signature backoff (+0.000) recorded as negative results; `filler`/`paraphrase` recommended for acceptance rather than repair.
- [Clean-bundle operational rehearsal, 30 August](evidence/OPERATIONAL_REHEARSAL_20260830.md): the archive runs and reproduces 0.878039 with no asset present; startup is 3.0 s with the asset against 49.4 s cold without it, and the unreleased SQLite handle blocks asset replacement outside tests.
