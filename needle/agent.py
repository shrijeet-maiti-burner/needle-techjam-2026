...
class Agent:
    ...
    def set_language(self, session_id: str, language: str | None) -> None:
        ...
        with an account locale, or a test. A pin wins over detection for the
        rest of the session, and an unsupported code is ignored rather than
        raising, because a bad locale string should not end a conversation.
        """
        if language is None:
            self._pinned_language.pop(session_id, None)
            return
    ...
    def _session_language(self, session_id: str, message: str) -> str:
        ...
        per-message and a later reply can be too short to carry evidence: "Sí,
        ...
        A pin always wins, and is re-read every turn so a caller can set it
    ...
    def _respond(
        ...
        if turn == 1:
            ...
            if not stated:
                ...
                # released simulator always states one. Measured on all 200
                # public sessions, every emitted message and every slate is
                # byte-identical with this branch present.
            ...
            # rather than guessing, so a lexicon noun that names several
            # categories, or none, leaves the bucket unqualified instead of
            # wrong. That is why the lexicon does not need its own resolver.
        ...
        if category_evidence:
            ...
            # the words even when the closed vocabulary cannot.
