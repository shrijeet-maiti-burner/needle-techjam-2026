...
def _join(values: Sequence[str], joiner: str = "and") -> str:
    ...
    if remainder > 0:
        named.append(f"+{remainder}" if joiner != "and" else f"{remainder} more")
    if not named:
        return ""
    if len(named) == 1:
        return named[0]
...
def _translated_message_for(record: dict, *, asking: bool, language: str) -> str:
    ...
    were tuned on the English sentence and a generic template would flatten
    them, so English keeps its own path, byte for byte, and only a customer who
    wrote in another language reaches this one.
    ...
    holds, because inventing a translation of a product attribute would be
    asserting something the catalog does not say. See needle/language.py.
    """
