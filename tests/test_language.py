...
CATALOG = Path(__file__).resolve().parents[1] / ".artifacts/participant-kit/techjam-conversational-search/data/catalog.jsonl"
...
class ResolutionTest(unittest.TestCase):
    """The lexicon recovers a noun; the catalog decides whether it is a key.

    An earlier revision of this work carried its own `resolve_coarse_category`.
    It is not here: `CatalogIndex.resolve_category` already resolves against the
    same closed vocabulary, with token-aligned fuzzy recovery and a uniqueness
    margin, so a second resolver would be a weaker duplicate of it. These
    assert the contract the lexicon depends on.
    """
    ...
    def setUpClass(cls) -> None:
        ...
        if not CATALOG.is_file():
            raise unittest.SkipTest("official catalog is not bootstrapped")
        cls.index = CatalogIndex(str(CATALOG))

    def test_a_lexicon_noun_reaches_a_catalog_category_or_declines(self) -> None:
        for message in ("Busco unas botas de cuero.", "Je cherche des bottes."):
            with self.subTest(message=message):
                words, _ = category_mention(message)
                self.assertTrue(words, "the lexicon recovered nothing")
                resolved = self.index.resolve_category(words)
                # Either a real catalog category or an honest decline. Never a
                # string the catalog does not file anything under.
                if resolved:
                    self.assertTrue(self.index.resolve_category(resolved))

    def test_an_ambiguous_noun_declines_rather_than_guessing(self) -> None:
        """"boots" is filed under several categories, so it is not a key."""
        self.assertEqual(self.index.resolve_category("boots"), "")

    def test_an_unknown_noun_resolves_to_nothing(self) -> None:
        self.assertEqual(self.index.resolve_category("rocket launcher parts"), "")
